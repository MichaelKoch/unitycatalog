...
RUN set -ex &&  apk update &&     apk upgrade &&     apk add bash git &&     rm -R /var/cache/apk/*
...
RUN set -ex && apk update && apk upgrade && apk add bash && rm -R /var/cache/apk/*
...
COPY ./start-uc-server "${UC_SERVER_BIN}"
...
USER root
RUN mkdir -p /opt/unitycatalog/etc/logs && \
    chmod 777 /opt -R
